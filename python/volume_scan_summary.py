...
def case_label(L, beta):
    return f"L{L}_beta_{beta:.1f}".replace(".", "p")
...
def blocked_jackknife_mean(xs, block_size):
    ...
    return jk_bias_corrected, jk_err
...
def main():
    ...
    print("L   beta   <P>_jk          err(P)          Veff(1,1)_jk     err           Veff(2,1)_jk     err")

    for L in LS:
        for beta in BETAS:
            folder = runs_dir / case_label(L, beta)

            sweeps, plaquette = read_plaquette_csv(folder / "plaquette.csv")
            kept_plaq = [p for s, p in zip(sweeps, plaquette) if s > THERMAL_CUT]
            plaq_res = blocked_jackknife_mean(kept_plaq, PLAQ_BLOCK_SIZE)
            if plaq_res is None:
                raise RuntimeError(f"Not enough plaquette data for L={L}, beta={beta}")
            plaq_jk, plaq_err = plaq_res

            wdata = read_wilson_csv(folder / "wilson_loops.csv")
    ...
    for beta in BETAS:
        xs = [r["L"] for r in results if r["beta"] == beta]
        ys = [r["plaq"] for r in results if r["beta"] == beta]
        es = [r["plaq_err"] for r in results if r["beta"] == beta]
        plt.errorbar(xs, ys, yerr=es, marker="o", capsize=3, label=fr"$\beta={beta}$")
    plt.xlabel("L")
    plt.ylabel(r"$\langle P \rangle$")
    plt.title("Plaquette vs L")
    plt.legend()
    plt.tight_layout()
    plt.savefig(runs_dir / "plaquette_vs_L.png", dpi=150)

    plt.figure(figsize=(8, 5))
    for beta in BETAS:
        xs = [r["L"] for r in results if r["beta"] == beta]
        ys = [r["v11"] for r in results if r["beta"] == beta]
        es = [r["v11_err"] for r in results if r["beta"] == beta]
        plt.errorbar(xs, ys, yerr=es, marker="o", capsize=3, label=fr"$\beta={beta}$, R=1")
    plt.xlabel("L")
    plt.ylabel(r"$V_{\mathrm{eff}}(1,1)$")
    plt.title(r"$V_{\mathrm{eff}}(1,1)$ vs L")
    plt.legend()
    plt.tight_layout()
    plt.savefig(runs_dir / "veff11_vs_L.png", dpi=150)

    plt.figure(figsize=(8, 5))
    for beta in BETAS:
        xs = [r["L"] for r in results if r["beta"] == beta]
        ys = [r["v21"] for r in results if r["beta"] == beta]
        es = [r["v21_err"] for r in results if r["beta"] == beta]
        plt.errorbar(xs, ys, yerr=es, marker="o", capsize=3, label=fr"$\beta={beta}$, R=2")
    plt.xlabel("L")
    plt.ylabel(r"$V_{\mathrm{eff}}(2,1)$")
    plt.title(r"$V_{\mathrm{eff}}(2,1)$ vs L")
    plt.legend()
    plt.tight_layout()
    plt.savefig(runs_dir / "veff21_vs_L.png", dpi=150)

    print("\nSaved:")
    print(runs_dir / "plaquette_vs_L.png")
    print(runs_dir / "veff11_vs_L.png")
    print(runs_dir / "veff21_vs_L.png")
